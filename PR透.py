...
data=open("D:/个人文件/大学/大二下/课程/热力学与统计物理/课程论文/QU算法模拟渗流/ER/data_ERPR2_1e7.txt",'w+')
print ("---File open---") 
...
while (r < r_max):
    ...
    while (t < t_max):
        ...
        i3 = random.randint(0,PR.N-1)
        i4 = random.randint(0,PR.N-1)
        

        pd1 = PR.sz[PR.rt[i1]]*PR.sz[PR.rt[i2]]#PR规则
        pd2 = PR.sz[PR.rt[i3]]*PR.sz[PR.rt[i4]]
...
while s3 < int(t_max/k):
    print("%d %.2f" % (s3*k,data_mean[s3]),file = data)
    s3 += 1
